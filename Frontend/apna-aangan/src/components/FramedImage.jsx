const FramedImage = ({ src, alt, color, className, overlayB, overlayT }) => {
  return (
    <div className={`framed-image ${className} relative`}>
      ...
      {overlayB && (
        <div
          className={`m-1 w-[99%] overlay absolute z-10 bottom-0 h-1/4 bg-gradient-to-t from-black/80 to-transparent`}
        ></div>
      )}
      {overlayT && (
        <div
          className={`m-1 w-[99%] overlay absolute z-10 top-0 h-4/5 bg-gradient-to-t to-black/60 from-transparent`}
        ></div>
      )}
      <img
